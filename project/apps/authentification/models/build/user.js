    ...
    insert: (pseudo, email, password, last_name, first_name, group) => new Promise((succes, fail) => {
        ...
        `;
            ...
            client.query(INSERT)
                .then((res) => {
                client.end();
                succes(new User(res[1].rows[0].currval,pseudo, email, hash, last_name, first_name, group));
                })
                .catch((err) => {
                client.end();
                fail(err);
    ...
    select: (condition) => new Promise((succes, fail) => {
        ...
        client.query(SELECT)
            ...
            for (let i = 0; i < res.rows.length; i += 1) {
                objects.push(new User(res.rows[i].id, res.rows[i].pseudo, res.rows[i].email, res.rows[i].password, res.rows[i].last_name, res.rows[i].first_name, res.rows[i].group_id));
            }
            client.end();
            succes(objects);
            })
