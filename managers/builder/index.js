...
const project = fs.readdirSync(`${__dirname}/../../project`, { withFileTypes: true });
const app = fs.readdirSync(`${__dirname}/../../app`, { withFileTypes: true });
...
async function createProject(projectName) {
  ...
  executeShell('npm install express bcrypt body-parser cors dotenv jsonwebtoken pg ejs eslint babel-eslint eslint-config-airbnb eslint-plugin-import eslint-plugin-jsx-a11y eslint-plugin-react eslint-plugin-react-hooks')
    .then(() => executeShell('npm install -g nodemon')
      .then(() => {
...
async function createHierarchy(projectName, path, dirs) {
  ...
  for (let i = 0; i < dirs.length; i++) {
    const newPath = `${path}/${dirs[i].name}`;
    const newPathSource = `${__dirname}/../../project/${newPath}`;
    const newPathDestination = `${projectName}/${newPath}`;
    if (dirs[i].isDirectory()) {
      ...
      await createHierarchy(
        projectName,
        newPath,
        await fs.readdirSync(newPathSource, { withFileTypes: true }),
      );
    } else {
      ...
      if (data.includes('[@]name')) {
        const newStrings = data.split('[@]name');
...
async function createHierarchyApp(appName, path, dirs) {
  ...
  for (let i = 0; i < dirs.length; i++) {
    const newPath = `${path}/${dirs[i].name}`;
    const newPathSource = `${__dirname}/../../app/${newPath}`;
    const newPathDestination = `apps/${appName}/${newPath}`;
    if (dirs[i].isDirectory()) {
      await fs.mkdirSync(newPathDestination);

      console.log(`${chalk.magenta(`- directory ${newPathDestination} created`)}`);
      await createHierarchyApp(appName,
        newPath,
        await fs.readdirSync(newPathSource, { withFileTypes: true }));
    } else {
      await fs.copyFileSync(newPathSource, newPathDestination);
      ...
      if (data.includes('[@appName]')) {
        const newStrings = data.split('[@appName]');
        for (let i = 0; i < newStrings.length - 1; i++) {
...
const executeShell = (command) => new Promise((succes) => {
  exec(command, (error) => {
    if (error) {
...
const connectRouter = (app) => new Promise((succes, fail) => {
  fs.readFile('./routes/index.js', 'utf8', (err, data) => {
    ...
    if (data.includes('/* End Routers Import */') && data.includes('/* End Routers Use */')) {
      ...
      const newData = `${spliter[0]}const ${app.charAt(0).toUpperCase() + app.slice(1)} = require('../apps/${app}/routes');
...
/* End Routers Use */${subSpliter[1]}`;
      ...
      fail(new Error('Balise End Routers or End Routers Use Import not found !'));
    }
  });
});
const connectDB = (app) => new Promise((succes, fail) => {
  fs.readFile('./db.js', 'utf8', (err, data) => {
    ...
    if (data.includes('/* End Import Models */') && data.includes('/* AUTOMATIC SYNC */')) {
      ...
      const newData = `${spliter[0]}const ${app.charAt(0).toUpperCase() + app.slice(1)}Models = require('./apps/${app}/models');
/* End Import Models */${subSpliter[0]}${app.charAt(0).toUpperCase() + app.slice(1)}Models.sync().then(
    ()=>{
      ...
      fail(new Error('Balise End Import Models or AUTOMATIC SYNC not found !'));
    }
  });
});
const connectBuilder = (app) => new Promise((succes, fail) => {
  fs.readFile('./build.js', 'utf8', (err, data) => {
    ...
    if (data.includes('/* End Import Models */') && data.includes('/* AUTOMATIC SYNC */')) {
      const spliter = data.split('/* End Import Models */');
      const subSpliter = spliter[1].split('/* AUTOMATIC SYNC */');
      const newData = `${spliter[0]}const ${app.charAt(0).toUpperCase() + app.slice(1)}Builder = require('./apps/${app}/main');
