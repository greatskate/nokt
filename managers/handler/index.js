    ...
    if (err) {
      fs.readFile(`${__dirname}/parse_file.js`, 'utf8', (err, data) => {
        ...
        fs.writeFile(`build/${model.nameModel.toLowerCase()}.js`, newData, (err) => {
          ...
          fs.readFile('./index.js', 'utf8', (err3, data) => {
            if (err3) {
              throw err;
            }
            if (data.includes('/* Handlers Import End */') && data.includes('/* Export REST */')) {
              ...
              const newData = `${spliter[0]}const { ${model.nameModel.charAt(0).toUpperCase() + model.nameModel.slice(1)}RestHandlers } = require('./build/${model.nameModel.toLowerCase()}');
